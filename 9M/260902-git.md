...
- **`git remote add origin 원격저장소주소`** : 원격저장소를 등록하는 명령어 .
...
- **`git clone 원격저장소주소`** : 원격저장소에서 내 로컬 폴더에 새롭게 생성.
...
- `git remote set-url 이메일주소` 명령어는 지금 적는 이메일 주소로 remote repository가 변경된다.
...
## github
6. `git remote add origin 이메일주소` 명령어로 remote repository를 등록한다. `git remote`로 잘 연결 되었는지 확인한다.
