...
const updatedBooks = addedBooks.map(price => price.id === 1 ? {...price, price: 36000} : price);

const inStockBooks = addedBooks.filter(instock => instock.inStock > 0);
const availableTitles = inStockBooks.map(titleName => titleName.title);

const calculateTotal = (...prices) => (prices.reduce((sum, current) => sum + current, 0));

const bookPrices = updatedBooks.map(price => price.price);

const totalBookPrice = calculateTotal(...bookPrices);

console.log(`원본 도서 수: ${books.length}`);
console.log(`추가 후 도서 수: ${addedBooks.length}`);
console.log(`원본 1번 도서 가격: ${addedBooks[0].price}`);
console.log(`할인된 1번 도서 가격: ${updatedBooks[0].price}`);
console.log('구매 가능 도서: ', availableTitles);
console.log(`전체 도서 가격 합계: ${totalBookPrice}`);